let login1 = <HTMLInputElement>document.getElementById('login');
let password1 = <HTMLInputElement>document.getElementById('password');
...
(event.target as HTMLButtonElement).classList.contains('edit') ? editUser(event) :
(event.target as HTMLButtonElement).classList.contains('delete') ? deleteUser(event) :
0;
...
function ableButton() {
  let testAdd = <HTMLInputElement>document.getElementById("add-user");
  let testEdit = <HTMLInputElement>document.getElementById("edit-user");
  if(passLogin.test(login1.value) && passPassword.test(password1.value) && passEmail.test(email1.value)) {
